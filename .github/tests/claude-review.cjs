...
(async()=>{
  ...
  const primaryRestoresCompletedRerun = await companionTest(exactJob,'Claude review PR #4 @ resolve-head',head,base,null,true,true,'nikejshah',{newerSequence:[null,'nikejshah','nikejshah'],statusSequence:[[],[newerPassStatus,newerFailedStatus]],recordPostedStatuses:true,actionOutcome:'failure'});
  assert.deepEqual(primaryRestoresCompletedRerun.filter(p=>p.url.includes('/statuses/')).map(p=>p.body.state), ['failure','success'], 'primary restores the latest PASS after an older failed attempt');
  const companionRestoresCompletedRerun = await companionTest(exactJob,'Claude review PR #4 @ resolve-head',head,base,null,true,false,'nikejshah',{newerSequence:[null,'nikejshah','nikejshah'],statusSequence:[[],[newerPassStatus,newerFailedStatus]],recordPostedStatuses:true});
  assert.deepEqual(companionRestoresCompletedRerun.filter(p=>p.url.includes('/statuses/')).map(p=>p.body.state), [], 'companion preserves the latest PASS after an older failed attempt');
  ...
  for (const primary of [true, false]) {
    const completed = await companionTest(exactJob,'Claude review PR #4 @ resolve-head',head,base,null,true,primary,'nikejshah',{newerSequence:[null,'nikejshah','nikejshah'],statusSequence:[[],[newerPassStatus,newerPendingStatus]],recordPostedStatuses:true,...(primary ? {actionOutcome:'failure'} : {})});
    assert.deepEqual(completed.filter(p=>p.url.includes('/statuses/')).map(p=>p.body.state), primary ? ['failure','success'] : [], 'normal pending-to-PASS history preserves the newer successful review');
  }
